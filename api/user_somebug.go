...
func UserRegister_bug(ctx *gin.Context) {
	var servicer service.UserRegisterCatcher
	if err := ctx.BindJSON(&servicer); err == nil {
		if res := servicer.RegisterBug(); serialization.GetResStatus(res) != 200 {
...
// @Failure 404 {object} serialization.Response{}
func UserLogin_bug(ctx *gin.Context) {
	var servicer service.UserLoginCatcher
	if err := ctx.BindJSON(&servicer); err == nil {
		// json get successfully
		if res := servicer.LoginBug(); serialization.GetResStatus(res) != 200 {
			// 收到json, 但是登入失敗
			// http code 403 => Forbidden 用戶端無訪問權限
			ctx.JSON(http.StatusForbidden, res)
		} else {
